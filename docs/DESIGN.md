...
The loop measures the current baseline, validates whole-file replacement paths for engine.py and kernels/*.py, writes complete contents (empty strings delete only kernel files), runs guard, the repair stage, public correctness, and the selected three-sample benchmark. A keep requires every gate, greater than 1 percent geomean improvement, and no selected workload more than 3 percent below its baseline throughput (added after experiment #37 won the local geomean and lost on Dryft). Five reverts at an item prohibit another attempt. Exact prompt-lookup speculation requires every earlier playbook item kept. A finally block restores rejected or interrupted experiments, and snapshots remain on disk for hard-crash recovery.

The repair stage sits between guard and check. After a proposal passes guard, `unit_test_remote` runs `tests/test_hand_rolled_kernels.py` and `tests/test_hand_rolled_handoff.py` on L4 against the candidate engine: every kernel against the Transformers 4.51.3 module it replaces on random inputs, and the full engine's prefill handoff, graph and buffer reuse, exact output counts and fresh-prompt reset on the pinned checkpoint. The suite runs in a fresh interpreter so a warm container never reuses a cached `kernels` module across turns, and the harness always supplies its own test files; a proposal cannot ship tests. On failure the same model receives the conversation so far, its own proposal as an assistant turn, and a bounded excerpt of the test output (test headers, failure blocks and the summary; passing comparison prints are dropped), and is asked for corrected complete files in the same schema. Corrected files are applied on top of the tested candidate, guarded and retested; a repair that fails guard or is not valid JSON consumes a turn without touching the engine. After two failed repair turns the experiment is logged as reverted with the note `failed after 2 repairs`; only a candidate that passes the unit tests reaches check and the judge. Each log line records `repairs` and `first_error`, which are journaled in `transaction.json` as they happen so an interrupted repair is recovered with the same fields. Raw test output is saved under `results/unit_tests/<id>-<turn>.json` and repair proposals under `results/proposals/<id>-repair<turn>.json`. The stage is diagnostic: it is not a latency, correctness or keep gate, and the sweep's numeric candidates bypass it. Source: `neokernel/loop.py`, `neokernel/modal_app.py` and `tests/test_loop.py`.
